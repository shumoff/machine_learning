...
def estimate():
    ...
    output_indexes = []
    changed_indexes = []
    with open("transport_output_3.txt", 'r') as f:
        for row in f.read():
            if row != '\n':
    ...
    for counter, value in enumerate(unknown_set["signals"]):
        ...
        if (x_1_list[0] < value["longitude"] < x_2_list[0]) and (y_1_list[0] < value["latitude"] < y_2_list[0]):
            ...
            if unknown_signals[counter] != 0:
                ...
                print('I HERE 0')
                square_incorrect += 1
                unknown_signals[counter] = 0
            # else:
            #     output_indexes.append(counter + 1)
        # red
    ...
    print("Ratio: ", ratio)
    # for i, j in enumerate(output_indexes):
    #     if i % 5 == 0:
    #         if unknown_signals[j-1] == 0:
    #             unknown_signals[j-1] = 1
    #         elif unknown_signals[j-1] == 1:
    #             unknown_signals[j-1] = 2
    #         elif unknown_signals[j-1] == 2:
    #             unknown_signals[j-1] = 0
    #         changed_indexes.append(j)
    #         if i == 100:
    #             break
    # with open('changed_indexes.txt', 'w', encoding='utf-8') as f:
    #     for index in changed_indexes:
    #         f.write(str(index) + "\n")
    with open('transport_output.txt', 'w', encoding='utf-8') as f:
        for signal in unknown_signals:
            f.write(str(signal) + "\n")
    return ratio
